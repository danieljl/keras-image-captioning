...
class BasicInference(object):
    ...
    def evaluate_training_set(self, include_prediction=False):
        return self._evaluate(
            self.predict_training_set(include_datum=True),
            include_prediction=include_prediction)

    def evaluate_validation_set(self, include_prediction=False):
        return self._evaluate(
            self.predict_validation_set(include_datum=True),
            include_prediction=include_prediction)

    def evaluate_testing_set(self, include_prediction=False):
        return self._evaluate(
            self.predict_testing_set(include_datum=True),
            include_prediction=include_prediction)
    ...
    def _predict_batch(self, X, y):
        ...
        captions_pred_str = self._preprocessor.decode_captions(
            captions_output=captions_pred, captions_output_expected=y)
...
def main(training_dir, method='beam_search', beam_size=3):
    ...
    metrics_path = os.path.join(training_dir, 'validation-metrics-old.yaml')
    predictions_path = os.path.join(training_dir,
                                    'validation-predictions-old.yaml')
